        ...
        public boolean wordBreak2(String s, List<String> wordDict) {
            ...
            for (int i = 0; i < s.length(); i++) {
                for (String word: wordDict) {
                    ...
                    //状态：dp[i] 表示子串 s[0:i] （即长度为 i 的子串，其实就是前缀）可以被空格拆分，并且拆分以后的单词是否落在 wordDict 中。
                    //后面的部分在 wordDict 中，前面的部分又满足题意，那么长度为 i 的 s 的前缀子串就满足题意
...
//leetcode submit region end(Prohibit modification and deletion)
    ...
    public static void main(String[] args) {
        leetcode.question.dfs.LeetCode_139_WordBreak.Solution solution = new leetcode.question.dfs.LeetCode_139_WordBreak().new Solution();
