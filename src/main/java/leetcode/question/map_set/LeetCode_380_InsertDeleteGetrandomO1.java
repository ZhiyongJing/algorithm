 ...
 * 3. 对于插入操作，我们首先检查值是否已经存在于哈希表中，如果存在则返回 false；否则，将值插入到数组的末尾，并在哈希表中添加值到索引的映射。
 * 4. 对于删除操作，我们首先检查值是否存在于哈希表中，如果不存在则返回 false；否则，找到该值在数组中的索引，并将数组末尾的元素移动到该位置，然后更新该元素在哈希表中的索引，最后从数组和哈希表中删除该值。
