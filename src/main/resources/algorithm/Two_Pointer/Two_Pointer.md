...
> | ----------------------------------------------- | ------------------------------ | ---------------------------------- | ------------------------------------------- |
> | **同向双指针（滑动窗口）**                | ✅ 适用                        | ❌ 可能不适用(窗口size不固定)      | 滑动窗口、最长/最短子数组（仅适用于非负数） |
...
> | **数组包含负数**（如 LeetCode 325）             | ❌**不适用**         | `sum` 可能增大也可能减小，无法保证 `left` 指针移动的正确性，需要用动态规划（如 Kadane 算法） |
> | **数组包含负数**（如 LeetCode 643）固定长度窗口 | ✅ 适用                    | 只是要求 **固定大小 k 的连续子数组和** ，那就每次减去窗口最左边元素，加上新进入的元素     |
