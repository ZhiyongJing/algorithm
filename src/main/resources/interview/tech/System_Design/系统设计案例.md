...
> | **PostgreSQL (Persistent Logs)**   | 存储长期登录日志                      |
...
### 19. Design distributed task scheduler
...
### 25. 设计一个信用卡联名program，考虑数据安全和高可用性
...
> # **🚀 设计一个银行交易额度检查系统（Check Transaction System）**
> **目标**：设计一个高效、安全、可扩展的银行间交易额度检查系统，确保银行在交易时不会超出设定的交易限额。  
>
> ---
>
> # **1️⃣ Functional Requirements（功能需求）**
> ### **🎯 核心功能**
> 1. **额度检查（Check Limit）**
>    - 每个银行有**每日/每月交易限额**。
>    - 交易前，系统需要**检查是否超出限额**，如果额度不足，则拒绝交易。
>
> 2. **交易处理（Process Transaction）**
>    - 交易符合额度要求时，系统**扣除对应银行的额度**并**确认交易**。
>    - 交易完成后，更新银行的**可用额度**。
>
> 3. **历史查询（Transaction History）**
>    - 系统允许查询某个银行的**历史交易记录**（时间、金额、交易对手）。
>
> 4. **异常处理（Error Handling）**
>    - 交易失败时，提供**详细的错误信息**（额度不足、系统错误、数据库故障等）。
>
> ---
>
> # **2️⃣ Non-Functional Requirements（非功能需求）**
> ### **⏳ 性能（Performance）**
> - 交易检查**低延迟**（目标 <100ms），确保系统能够快速响应请求。
> - **高并发支持**，需要支持**每天上百万级交易请求**。
>
> ### **📈 可扩展性（Scalability）**
> - 10w+ 银行，每个银行可能与多个银行交易，系统必须支持**横向扩展**（Scale-out）。
>
> ### **🔄 数据一致性（Data Consistency）**
> - 交易涉及**资金流动**，必须保证**强一致性**，避免**超额交易**或**数据丢失**。
> - 采用**ACID 事务**或**分布式事务**（如 Saga Pattern）。
>
> ### **📊 监控 & 日志（Monitoring & Logging）**
> - 需要**实时监控**系统健康状态，**检测异常交易**。
> - **日志记录**所有交易请求，确保**可审计**。
>
> ---
>
> # **3️⃣ Services & Components（系统组件）**
> ### **🔹 主要服务**
> | Service                     | 作用                                          |
> | --------------------------- | --------------------------------------------- |
> | **Transaction Service**     | 处理交易请求，检查额度，更新余额              |
> | **Rate Limiting Service**   | 限制短时间内某个银行的交易频率，防止 DoS 攻击 |
> | **Audit & Logging Service** | 记录交易日志，提供历史查询                    |
> | **Alerting & Monitoring**   | 监控系统异常，防止欺诈交易                    |
> | **Cache Service**           | 缓存热点银行额度，减少数据库访问              |
>
> ---
>
> # **4️⃣ 数据库选型**
> | 数据类型            | 适用数据库                                           |
> | ------------------- | ---------------------------------------------------- |
> | 交易额度 & 账户信息 | **PostgreSQL / MySQL**（ACID 事务保证一致性）        |
> | 交易日志 & 历史记录 | **MongoDB / Elasticsearch**（支持大规模查询 & 分析） |
> | 高并发额度检查      | **Redis**（缓存热点数据，加速查询）                  |
>
> 📌 **为什么使用 Redis？**
> - 交易检查**高频查询**，用 Redis 缓存**当前额度**，减少数据库访问压力。
> - 使用 **Redis 分布式锁** 确保**并发修改额度时的一致性**。
>
> ---
>
> # **5️⃣ 数据库表设计**
...
>
> 📌 **优化方案**
> - `remaining_limit` 存 Redis，**减少数据库查询**。
> - `transactions` **按时间+银行 ID 分区**，提高查询性能。
>
> ---
>
> # **6️⃣ 面试可能被问的问题**
...
> ✅ **使用 Redis + 分布式锁**
> ```java
> String lockKey = "lock:bank:" + bankId;
> boolean acquired = redisLock.tryLock(lockKey, 10);
> if (acquired) {
>    // 检查额度 & 扣款
...
> ```sql
> BEGIN;
> SELECT remaining_limit FROM banks WHERE bank_id = 1 FOR UPDATE;
> UPDATE banks SET remaining_limit = remaining_limit - 100 WHERE bank_id = 1;
> COMMIT;
> ```
>
> ---
...
> - API 层：使用 **负载均衡**（AWS ALB / Nginx）。
> - 数据层：**分库分表**，如按银行 ID **分片**。
> - 缓存层：使用 **Redis Cluster** 处理**高并发查询**。
>
> ✅ **异步处理（Asynchronous Processing）**
> - **交易检查 -> Kafka -> 交易确认**
> - **Kafka + Consumer** 处理交易，避免主线程阻塞。
>
> ---
>
> ### **Q3: 如何防止欺诈交易（Fraud Detection）？**
> ✅ **风控策略**
> - **Rate Limiting**（防止单个银行短时间内大量交易）。
> - **AI 异常检测**（机器学习检测异常交易）。
> - **多因素身份验证（MFA）**（高风险交易要求二次验证）。
>
> ✅ **实时监控**
> - 结合 **Kafka + Elasticsearch** 实时分析交易模式，发现异常交易立即报警。
>
> ---
>
> # **7️⃣ 如何扩展系统（Scaling Strategies）**
> ✅ **前端层**
> - **API Gateway 负载均衡**
> - **CDN 加速 API 请求**
>
> ✅ **后端层**
> - **水平扩展（Scale-out）**：部署多个 `Transaction Service` 实例，使用负载均衡。
> - **分布式缓存（Redis Cluster）**：缓存热点银行额度，减少数据库压力。
>
> ✅ **数据库层**
> - **读写分离（Master-Slave）**
> - **分库分表（Sharding）**
> - **NoSQL 存储**（MongoDB 存历史交易数据）
>
> ✅ **异步架构**
> - **Kafka 消息队列** 处理交易，避免 API 直接写数据库导致性能瓶颈。
>
> ---
>
> # **🎯 总结**
> 📌 **核心架构**
> - **API 层**：处理请求，调用 **Transaction Service**。
> - **交易服务（Transaction Service）**：处理额度检查 & 交易执行。
> - **数据库层（MySQL + Redis）**：存储额度 & 交易历史。
> - **缓存 & 限流**：使用 **Redis** 缓存额度，并进行 **Rate Limiting**。
> - **可扩展性**：使用 **Kafka、微服务、分布式锁** 提升系统并发能力。
>
> 🚀 **最终目标**：高效、安全、可扩展的银行交易额度检查系统！

### 27. Design portfolio process

> ## **🚀 System Design: ETF & Portfolio Query System**
> 📌 **目标**：  
> 设计一个系统，允许用户查询**某支股票在哪些基金公司（Fund Manager）的哪些 ETF 组合（Portfolio）中被持有，以及持有的股数（Share）**。
>
> ---
>
> ## **1️⃣ Functional Requirements（功能需求）**
> ✅ **支持 CSV 数据导入**  
>    - 用户可以上传 ETF 数据（包含 `Fund Manager`、`Portfolio`、`Stock`、`Share`）。
>
> ✅ **支持 API 查询**  
>    - 用户可以**输入某个股票名称**（如 `IBM`），查询：
>      - 哪些 **基金公司**（Fund Manager）持有它？
>      - 具体在哪些 **ETF 组合**（Portfolio）？
>      - 每个 ETF 组合持有多少股（Share）？
>
> ✅ **高效的查询性能**  
>    - 需要优化查询，使其能**快速响应用户请求**（目标 <100ms）。
>
> ---
>
> ## **2️⃣ Non-Functional Requirements（非功能需求）**
> ✅ **高可用性（High Availability）**  
>    - 需要确保 API **7x24 小时运行**，避免查询中断。  
>
> ✅ **低延迟（Low Latency）**  
>    - 查询需要**快速响应**，特别是大规模数据（百万级 ETF 持仓记录）。  
>
> ✅ **可扩展性（Scalability）**  
>    - 需要支持**不断增长的 ETF 数据**，系统必须能够水平扩展。  
>
> ✅ **数据一致性（Data Consistency）**  
>    - ETF 数据需要保持**高一致性**，避免查询到过时的数据。  
>
> ✅ **实时更新支持（Real-time Update）**  
>    - ETF 组合的持仓信息会**定期更新**，系统需要支持 **批量数据导入** 并 **不影响查询性能**。  
>
> ---
>
> ## **3️⃣ Services & Components（系统组件）**
...
> | Service                          | 功能                                               |
> | -------------------------------- | -------------------------------------------------- |
> | **Data Ingestion Service**       | 处理 CSV 数据导入，将 ETF 持仓数据存入数据库       |
> | **Portfolio Query API**          | 提供股票查询 API，返回基金公司、ETF 组合、持仓信息 |
> | **Cache Service (Redis)**        | 缓存热门查询股票的结果，加速查询                   |
> | **Monitoring & Logging Service** | 记录查询日志，监控 API 性能和数据更新情况          |
>
> ### **🔹 主要组件**
> | 组件            | 技术栈                                                    |
> | --------------- | --------------------------------------------------------- |
> | **API Gateway** | AWS API Gateway / Kong / Nginx                            |
> | **数据库**      | PostgreSQL（关系型数据库）+ Elasticsearch（全文搜索优化） |
> | **缓存**        | Redis（存储热门股票的查询结果）                           |
> | **消息队列**    | Kafka / RabbitMQ（异步数据更新）                          |
> | **负载均衡**    | AWS ELB / Nginx                                           |
...
> | 数据类型     | 数据库                                  |
> | ------------ | --------------------------------------- |
> | ETF 持仓数据 | **PostgreSQL / MySQL（ACID 事务保障）** |
> | 股票查询索引 | **Elasticsearch（优化模糊搜索）**       |
> | 热门查询缓存 | **Redis（加速查询）**                   |
>
> 📌 **为什么选 PostgreSQL？**
> - **支持 SQL 复杂查询**（适合 ETF 数据结构）。
> - **支持 JSONB**（可以存储 ETF 的额外信息）。
> - **强一致性**（保证 ETF 数据不会丢失）。
>
> 📌 **为什么需要 Elasticsearch？**
> - **支持高效的全文搜索**（股票模糊查询）。
...
>
> 📌 **为什么用 Redis？**
> - **缓存热门股票查询**，减少数据库压力，提高响应速度。
>
> ---
>
> ## **5️⃣ 数据库表设计**
> ### **🔹 `etf_holdings`（ETF 持仓表）**
> ```sql
> CREATE TABLE etf_holdings (
>     id SERIAL PRIMARY KEY,
>     fund_manager VARCHAR(255) NOT NULL,  -- 基金公司
>     portfolio VARCHAR(255) NOT NULL,     -- ETF 组合名称
>     stock VARCHAR(255) NOT NULL,         -- 股票名称
>     shares INT NOT NULL,                 -- 持有股数
>     last_updated TIMESTAMP DEFAULT CURRENT_TIMESTAMP  -- 更新时间
> );
> ```
> 🔹 **索引优化**
> ```sql
> CREATE INDEX idx_stock ON etf_holdings(stock);
> ```
> - 使用 `stock` 作为索引，加快查询速度。
>
> ---
>
> ## **6️⃣ 面试可能被问的问题**
...
>
> ✅ **缓存优化**
> - **Redis 缓存热门查询**（如 `IBM`，每天有大量查询）。
> - **LRU 机制** 自动清除不常用数据。
>
> ✅ **读写分离**
> - **数据库主从架构**（Master-Slave），**查询从库，写入主库**。
>
> ---
>
> ### **Q2: 如何保证数据一致性？**
...
> ```
> ✅ **异步数据更新**
> - **Kafka + Worker** 处理 CSV 导入，**避免影响查询性能**。
>
> ---
>
> ### **Q3: 如何扩展系统？（Scalability）**
> ✅ **水平扩展（Scale-out）**
> - **API 层**：使用 **负载均衡（Nginx / AWS ELB）**。
> - **数据库层**：
>   - **分库分表**（Sharding）按 `fund_manager` 切分数据库。
>   - **读写分离**，查询使用 Read Replica（只读副本）。
> - **缓存层**：
>   - **Redis Cluster** 处理**高并发查询**。
>
> ✅ **分布式架构**
> - **多个 API Server 并行处理请求**。
> - **Kafka + Worker 处理数据导入**，避免影响 API 查询性能。
>
> ---
>
> ### **Q4: 如何处理实时更新（Real-time Update）？**
> ✅ **批量导入 vs 流式更新**
> - **批量导入（Bulk Insert）**：
>   - 适用于**每天一次大规模数据更新**。
>   - 先存入**临时表**，然后**批量替换正式表数据**。
>
> - **流式更新（Streaming）**：
>   - 适用于**小规模增量更新**。
>   - 使用 **Kafka + CDC（Change Data Capture）** 实时同步 ETF 数据。
>
> ---
>
> # **7️⃣ 如何扩展系统（Scaling Strategies）**
...
>
> ✅ **后端层**
> - **水平扩展（Scale-out）**：多个 `Portfolio Query Service` 实例，使用负载均衡。
> - **分布式缓存（Redis Cluster）**：缓存热点 ETF 查询结果。
>
> ✅ **数据库层**
> - **读写分离（Master-Slave）**
> - **分库分表（Sharding）**
> - **NoSQL 存储（Elasticsearch）** 用于全文搜索。
>
> ✅ **异步架构**
> - **Kafka 消息队列** 处理 ETF 数据导入，避免影响 API 直接写数据库导致性能瓶颈。
>
> ---
>
> ## **🎯 总结**
> 📌 **核心架构**
> - **API 层**：处理请求，调用 **Portfolio Query Service**。
> - **查询服务（Portfolio Query Service）**：提供股票查询 API。
> - **数据库层（PostgreSQL + Elasticsearch）**：存储 ETF 数据，优化查询索引。
> - **缓存（Redis）**：缓存热门股票查询，减少数据库压力。
> - **可扩展性**：使用 **Kafka、微服务、分布式存储** 提升系统并发能力。
>
> 🚀 **最终目标**：高效、可扩展的 ETF 组合查询系统！



### 28. Design 超市购物系统

> # **🚀 设计一个超市购物系统（Supermarket Shopping System）**
> **目标**：  
> 构建一个**高效、可扩展、低延迟**的超市购物系统，支持商品管理、库存管理、购物车、订单结算、支付等功能。
>
> ---
>
> ## **1️⃣ Functional Requirements（功能需求）**
...
>
> ### **🔹 购物车**
> - 用户可以**添加商品到购物车**、**调整数量**、**删除商品**。
> - 购物车应 **实时计算总价**。
>
> ### **🔹 订单 & 结算**
> - 用户可以 **创建订单**。
> - 系统需要 **处理支付**，支持信用卡、PayPal、微信、支付宝。
...
> ## **2️⃣ Non-Functional Requirements（非功能需求）**
> ### **🔹 高可用性（High Availability）**
> - **7x24 小时可用**，即使在高峰期也能正常运行。
>
> ### **🔹 高并发 & 低延迟（High Concurrency & Low Latency）**
> - 查询 & 购物车操作 < **100ms** 响应时间。
> - **支持高并发 QPS（见 QPS 计算）**。
>
> ### **🔹 数据一致性（Data Consistency）**
...
> ---
>
> ## **3️⃣ QPS & Storage Calculation（QPS & 存储计算）**
> ### **🔹 QPS 计算**
> 假设系统支持 **1M 日活跃用户（DAU）**：
> 1. **商品查询**：QPS = **1,000**（每人每天查询 10 次）
> 2. **购物车操作**：QPS = **500**（30% 用户每天 5 次）
> 3. **订单创建**：QPS = **200**（10% 用户每天 2 次）
> 4. **支付处理**：QPS = **100**（50% 订单完成支付）
> 5. **库存扣减 & 订单状态更新**：QPS = **100**
>    
>
> 📌 **总 QPS 预计 = 2,000 - 5,000**
>
> ---
>
> ### **🔹 存储计算**
> 假设系统有 **10M 商品**，每天 **100K 订单**：
> 1. **商品表** (`products`)：
>    - 10M 条记录，每条 **1KB** → **存储需求 ≈ 10GB**
> 2. **购物车表** (`cart_items`)：
>    - 活跃用户 1M，每人 5 个商品，每条 512B → **存储需求 ≈ 2.5GB**
> 3. **订单表** (`orders`)：
>    - 每天 100K 订单，每个订单 1KB → **一年存储 ≈ 36GB**
> 4. **支付记录** (`payments`)：
>    - 每天 100K 订单，每个支付记录 500B → **一年存储 ≈ 18GB**
>
> 📌 **数据库存储需求（年）** ≈ **80-100GB**
>
> ---
>
> ## **4️⃣ Services & Components（系统组件）**
> | Service                          | 作用                                     |
> | -------------------------------- | ---------------------------------------- |
> | **User Service**                 | 处理用户注册、登录、权限管理             |
> | **Product Service**              | 管理商品（增删改查、库存管理）           |
> | **Cart Service**                 | 购物车管理（增删改商品，计算总价）       |
> | **Order Service**                | 订单管理（创建订单、支付、更新状态）     |
> | **Payment Service**              | 支付管理（处理支付请求，集成第三方支付） |
> | **Inventory Service**            | 库存管理（扣减库存，防止超卖）           |
> | **Promotion Service**            | 促销 & 优惠券管理                        |
> | **Logging & Monitoring Service** | 记录用户行为 & 监控系统健康              |
>
> ---
>
> ## **5️⃣ 数据库选型**
> | 数据类型          | 数据库                              |
> | ----------------- | ----------------------------------- |
> | 商品 & 订单       | **MySQL / PostgreSQL（ACID 事务）** |
> | 用户会话 & 购物车 | **Redis（低延迟存储）**             |
> | 搜索 & 推荐       | **Elasticsearch（支持全文检索）**   |
> | 支付 & 交易日志   | **MongoDB / Cassandra**             |
>
> 📌 **数据库分层**
> - **Redis** 缓存商品详情 & 购物车，减少数据库查询。
> - **MySQL** 存储订单 & 用户数据，支持事务。
> - **Elasticsearch** 提供商品搜索，提高查询速度。
>
> ---
>
> ## **6️⃣ 数据库表设计**
...
> ✅ **使用 Kafka 处理异步事件**
> ```sh
> order_created -> Kafka -> Payment Service
> ```
> ✅ **分库分表（Sharding）**
> - **按 `user_id` 进行分库**，不同用户的订单存储在不同数据库。
>
> ---
>
> ## **8️⃣ 如何扩展系统（Scaling Strategies）**
> ✅ **前端层**
> - **API Gateway 负载均衡**
> - **CDN 缓存静态资源**
>
...
> - **MySQL 读写分离**
> - **分库分表（Sharding）**
> - **NoSQL（Elasticsearch）** 用于商品搜索
>
> ✅ **异步架构**
> - **Kafka 消息队列** 处理库存扣减、订单状态更新，避免 API 阻塞。
>
> ---
>
> ## **🎯 总结**
...
> - **支持横向扩展**
>
> 🚀 **最终目标**：打造**高效、可扩展、用户体验流畅的超市购物系统！** 🎯

### 29. Design 机场雷达

> 要求如下：可以显示所有的飞机；highligh那些靠的很近，有可能会撞上的飞机；可以track飞机的进入和离开airspace
>
> # **🚀 设计一个飞机雷达系统（Airplane Radar System）**
>
> 📌 **目标**：  
> 设计一个**实时飞机监控雷达系统**，支持：
> 1. **显示所有飞机** 🛫
...
> ### **🔹 主要功能**
> ✅ **实时显示所有飞机的位置**  
>    - 通过雷达 **实时更新飞机的经纬度、高度、速度**。  
> ✅ **检测可能碰撞的飞机（Collision Alert）**  
>    - **计算飞机之间的距离**，若低于安全阈值（如 5km），发出 **Collision Alert**⚠️。  
> ✅ **追踪飞机进出空域**  
>    - 记录 **飞机进入/离开** 监控范围（Airspace）。  
> ✅ **飞机信息管理**  
>    - 显示 **航班号、飞机型号、目的地、高度、速度** 等信息。  
> ✅ **支持多雷达同步（Multi-Radar Integration）**  
>    - 可接入**多个雷达站**，提高覆盖范围。  
> ✅ **数据存储 & 回放历史数据**  
>    - 支持查询 **过去 24 小时飞机轨迹**，方便调查。  
>
> ---
>
> ## **2️⃣ Non-Functional Requirements（非功能需求）**
...
>
> ### **🔹 低延迟（Low Latency）**
> - **每秒更新 10,000+ 飞机数据**，保证雷达实时性（目标 < **100ms**）。  
>
> ### **🔹 可扩展性（Scalability）**
> - 需要**支持全球范围**，能扩展到 **10,000+ 雷达站**，每个雷达可监控 **1000+ 飞机**。  
>
> ### **🔹 容错性（Fault Tolerance）**
> - 若某个雷达站宕机，**系统应自动切换到其他雷达站**，防止数据丢失。  
>
> ### **🔹 数据一致性（Data Consistency）**
> - **雷达数据需强一致性**，确保不会漏掉关键航班信息。  
>
> ---
>
> ## **3️⃣ QPS & Storage Calculation（QPS & 存储计算）**
> ### **🔹 QPS 计算**
> 假设：
> - **全球 10,000 个雷达站**，每秒更新 **1000 架飞机**。
> - **每架飞机的数据大小** ≈ **500B**（包括 `ID、位置、高度、速度`）。
> - 每秒处理数据量：  
>   ```sh
>   10,000 radar * 1,000 planes * 500B = 5GB/s
>   ```
>
> 📌 **峰值 QPS ≈ 10M QPS**（需要 Kafka + NoSQL 处理高并发）
>
> ---
>
> ### **🔹 存储计算**
...
>   ```
>
> 📌 **需要分布式存储（S3、HDFS、Cassandra）**，结合 **冷热数据分层** 降低成本。
>
> ---
>
> ## **4️⃣ 系统架构（Services & Components）**
> ### **🔹 主要服务**
> | Service                   | 作用                                   |
> | ------------------------- | -------------------------------------- |
> | **Radar Data Ingestion**  | 采集雷达数据，并存入数据库             |
> | **Collision Detection**   | 计算飞机之间的安全距离，触发警报       |
> | **Airspace Tracking**     | 追踪飞机进入/离开空域                  |
> | **Real-time Processing**  | Kafka + Streaming 计算，低延迟处理数据 |
> | **Visualization Service** | 提供 Web 界面，展示雷达数据            |
>
> ### **🔹 主要组件**
> | 组件         | 技术栈                                                       |
> | ------------ | ------------------------------------------------------------ |
> | **数据采集** | Kafka / RabbitMQ（流式数据处理）                             |
> | **数据库**   | **PostgreSQL**（航班 & 雷达元数据）+ **Cassandra**（高吞吐雷达数据存储） |
> | **缓存**     | Redis（存储热门航班数据，加速查询）                          |
> | **实时处理** | Apache Flink / Spark Streaming（计算飞机位置 & 碰撞检测）    |
> | **前端**     | WebSocket + React（实时展示飞机位置）                        |
>
> ---
>
> ## **5️⃣ 数据库选型**
> | 数据类型   | 数据库                                   |
> | ---------- | ---------------------------------------- |
> | 雷达数据   | **Cassandra / HBase**（支持高吞吐写入）  |
> | 航班元数据 | **PostgreSQL / MySQL**（存航班信息）     |
> | 轨迹历史   | **Elasticsearch**（支持时序查询 & 分析） |
> | 热门查询   | **Redis**（缓存热点航班数据）            |
>
> 📌 **数据库分层**
> - **Redis** 存**最近 10 分钟航班数据**，加速查询。
> - **Cassandra** 存**最近 30 天航班轨迹**。
> - **S3 / HDFS** 存**历史归档数据**。
>
> ---
>
...
>     altitude INT NOT NULL,   -- 高度（米）
>     speed INT NOT NULL,      -- 速度（km/h）
>     direction INT NOT NULL,  -- 方向（角度）
>     timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP
> ) WITH CLUSTERING ORDER BY (timestamp DESC);
> ```
>
> ### **🔹 `airspace_tracking`（空域追踪表）**
> ```sql
> CREATE TABLE airspace_tracking (
>     aircraft_id INT REFERENCES aircrafts(aircraft_id),
>     status ENUM('ENTERED', 'EXITED'),
>     timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP
> );
> ```
>
...
> ✅ **Redis 缓存最新的飞机数据**
> ```sh
> HSET "plane:flight_123" "lat" 37.7749 "long" -122.4194 "alt" 10000
> ```
> ✅ **Cassandra 存时序数据**
> ```sql
> SELECT * FROM radar_data WHERE aircraft_id = 123 ORDER BY timestamp DESC LIMIT 10;
> ```
>
> ---
>
...
> ✅ **计算两架飞机之间的 3D 距离**
> ```sql
> SELECT a1.aircraft_id, a2.aircraft_id,
>        SQRT(POW(a1.latitude - a2.latitude, 2) +
>             POW(a1.longitude - a2.longitude, 2) +
>             POW(a1.altitude - a2.altitude, 2)) AS distance
> FROM radar_data a1, radar_data a2
> WHERE a1.timestamp = a2.timestamp AND a1.aircraft_id != a2.aircraft_id
> HAVING distance < 5;
> ```
>
> ---
...
> ✅ **后端**
> - **Kafka 处理雷达数据流**
> - **Flink/Spark Streaming 实时计算碰撞检测**
> - **数据库水平扩展（Cassandra / Sharding）**
>
> ---
>
> ## **8️⃣ 最终架构**
> 1. **Kafka** 接收 **雷达数据**。
> 2. **Flink Streaming** 计算 **碰撞警报** & **航班轨迹**。
> 3. **Redis** 缓存 **最近 10 分钟数据**。
> 4. **Cassandra** 存 **30 天数据**，**Elasticsearch** 提供 **时序查询**。
> 5. **WebSocket + React** 实时可视化飞机位置。
>
> 🚀 **最终目标**：**高效、低延迟、可扩展的飞机雷达监控系统！** 🎯
...
> 📌 **目标**：  
> 构建一个 **高吞吐、低延迟、可扩展** 的 **实时交易数据处理系统**，能够：
> 1. **接收 100 个交易所（Exchanges）的实时交易数据**
> 2. **存储和处理交易数据**
> 3. **将数据分发给不同的消费端（计算引擎、交易终端等）**
>
> ---
>
> ## **1️⃣ Functional Requirements（功能需求）**
...
>
> ✅ **消息队列 & 订阅分发**
>    - **多 Consumer** 订阅不同交易所数据：
>      - 交易分析服务
>      - 交易终端 UI
>      - AI 交易模型
>    - **支持 Kafka Partitioning**，提高并发消费能力。
>
> ✅ **历史数据查询**
>    - **提供最近 24 小时交易数据查询**
>    - **支持过去 30 天交易数据存档**
...
> ✅ **高可用性（High Availability）**
>    - **7x24 小时运行**
>    - **多数据中心容灾（Multi-Region Deployment）**
>
> ✅ **低延迟（Low Latency）**
>    - **数据写入 & 处理 < 10ms**
>    - **数据查询 < 100ms**
...
> ✅ **数据一致性（Data Consistency）**
>    - **交易数据必须强一致性**
>    - **采用 Kafka + CDC（Change Data Capture）** 确保数据可靠性
>
> ✅ **高吞吐 & 并发处理**
>    - **Kafka Partitioning**，保证 Producer & Consumer 高并发。
>
> ✅ **安全性**
>    - **API 访问控制**
>    - **加密存储交易数据（AES256）**
...
> ### **🔹 QPS 计算**
> 假设：
> - **100 个交易所，每秒 5000 笔交易**
> - **每条交易数据 500B**
>
> 📌 **每秒数据量**：
> ```sh
> 100 exchanges * 5000 tx/s * 500B = 250MB/s
> ```
> 📌 **每天数据量**：
> ```sh
> 250MB/s * 86400s = 21.6TB/day
> ```
> 📌 **30 天数据存储**：
> ```sh
> 21.6TB * 30 = 648TB
...
> ---
>
> ## **4️⃣ System Architecture（系统架构 & 组件）**
> ### **🔹 核心组件**
> | Component                   | 作用                           |
> | --------------------------- | ------------------------------ |
> | **Kafka**                   | 交易数据流入（Multi-Producer） |
> | **Flink / Spark Streaming** | 实时计算 & 数据清洗            |
> | **Cassandra / HBase**       | 高吞吐时序数据存储             |
> | **Elasticsearch**           | 交易查询 & 分析                |
> | **Redis**                   | 缓存热点交易数据               |
> | **S3 / HDFS**               | 冷数据存储（归档）             |
> | **API Gateway**             | 供外部系统查询交易数据         |
>
> ---
>
> ## **5️⃣ 数据库选型**
> | 数据类型     | 数据库                                |
> | ------------ | ------------------------------------- |
> | 交易流数据   | **Kafka + Cassandra**（低延迟 NoSQL） |
> | 历史数据存储 | **S3 / HDFS**（降低成本）             |
> | 交易查询     | **Elasticsearch**（支持全文检索）     |
> | 热点数据     | **Redis**（缓存最近 10 分钟数据）     |
>
> ---
>
> ## **6️⃣ 数据库表设计**
...
> ---
>
> ## **7️⃣ End-to-End Data Flow（整体流程）**
> ### **📌 交易数据处理流程**
> 1️⃣ **交易所 API 发送数据（Producer）**  
>    - 100+ 交易所将交易数据写入 Kafka Topic。  
>    - **Kafka Partitioning**，按 `symbol` 分区，提高并发。  
>
> 2️⃣ **实时流处理（Flink / Spark Streaming）**
>    - **数据清洗**（去重、格式化）
>    - **计算市场快照**
>    - **异常交易检测（High-Frequency Trading, HFT）**
>    - **存入 NoSQL（Cassandra / HBase）**
>
> 3️⃣ **数据存储**
>    - **短期存储**（Redis / Elasticsearch）
>    - **长期存储**（S3 / HDFS）
>
> 4️⃣ **数据消费（Multi-Consumer）**
>    - **交易分析引擎**
>    - **实时图表 UI**
>    - **自动交易策略**
>
...
>
> ## **8️⃣ 面试常见问题 & 答案**
> ### **Q1: Kafka Partitioning 如何设计？**
> ✅ **按 `symbol`（交易对） 分区**
> ```sh
> Kafka Partitions: BTC/USD, ETH/USD, AAPL/USDT
> ```
> 📌 **每个交易对都有自己的分区，提高并发度**
>
> ---
...
> ✅ **Redis 缓存最近交易**
> ```sh
> HSET "trade:BTC/USD" "price" 45000 "volume" 10
> ```
> ✅ **Elasticsearch 存历史交易**
> ```sql
> SELECT * FROM trades WHERE symbol='BTC/USD' ORDER BY timestamp DESC LIMIT 100;
> ```
>
> ---
>
...
> - **Kafka 事务性 Producer**
> - **消费端幂等（Exactly Once Processing）**
>
> ---
>
> ## **9️⃣ 如何扩展系统（Scaling Strategies）**
> ✅ **Kafka Partitioning**
> - **更多交易所** → **增加 Partition**
> - **高频交易** → **增加 Broker**
>
...
>
> ## **🎯 最终架构**
> 1️⃣ **Kafka → Flink → Cassandra / Redis / Elasticsearch**  
> 2️⃣ **计算交易快照 & 异常检测**  
> 3️⃣ **实时分发数据 → 交易引擎 / 终端 UI / AI 策略**  
>
> 🚀 **最终目标**：**高吞吐、低延迟、可扩展的实时交易数据处理系统！** 🎯

### 31. Design hotel booking

> 我们来设计一个 **Hotel Booking System**，并按照你的要求一步步分析。
>
> ---
>
> ## **1. Functional Requirements（功能需求）**
> ### **Core Features（核心功能）**
> ✅ **Search for hotels** – 用户可以根据 **location, check-in/check-out date, guests, filters（price, rating, amenities）** 搜索酒店。  
> ✅ **View hotel details** – 每家酒店包含 **rooms availability, pricing, images, reviews, policies** 等信息。  
> ✅ **Make a booking** – 用户可以选择房型、日期，并完成预订。  
> ✅ **Cancel/Modify booking** – 允许用户取消/更改预订（依据酒店政策）。  
> ✅ **Payment processing** – 预订时支持 **credit card, PayPal, Amazon Pay** 付款。  
> ✅ **View booking history** – 用户可以查看自己的预订记录。  
> ✅ **Hotel management** – 酒店管理员可以 **add/update room availability, pricing, images, policies**。  
>
> ---
>
> ## **2. Non-Functional Requirements（非功能需求）**
> ✅ **Scalability（可扩展性）** – 系统应该能够支持 **millions of users & hotels**。  
> ✅ **Availability（高可用性）** – 确保搜索、预订服务不会因为单点故障导致系统崩溃（99.99% uptime）。  
> ✅ **Consistency（一致性）** – 预订时需要强一致性，防止 overbooking（超售）。  
> ✅ **Performance（高性能）** – 低延迟搜索（< 200ms），预订流程 < 2s。  
> ✅ **Security（安全）** – 保护用户支付信息、身份验证（OAuth, JWT, Multi-Factor Authentication）。  
>
> ---
>
> ## **3. QPS, Storage Calculation（QPS & 存储计算）**
> ### **假设**
> - **Users per month**: 50M  
> - **Daily Active Users (DAU)**: ~5M  
> - **Search Requests per second (QPS)**: ~50K  
> - **Booking Requests per second (QPS)**: ~5K  
> - **Average hotel size**: 200 rooms  
> - **Total hotels**: 1M  
> - **Average bookings per hotel per day**: 50  
> - **Booking history storage**: ~500B per record
>
> ### **Storage Calculation**
> - **Hotel Data**: 1M hotels * 5KB = **5GB**
> - **Room Availability**: 1M hotels * 200 rooms * 100B = **20GB**
> - **Bookings per year**: 1M hotels * 50 bookings * 365 days = **18.25B records**
>   - 18.25B * 500B = **9TB/year** for booking history
>
> ---
>
> ## **4. Services/Components Needed（所需服务/组件）**
> | Service                      | Function                                          |
> | ---------------------------- | ------------------------------------------------- |
> | **API Gateway**              | 统一管理 API 请求，流量控制（Amazon API Gateway） |
> | **Search Service**           | 处理酒店搜索（Elasticsearch）                     |
> | **Booking Service**          | 处理预订逻辑，保证一致性                          |
> | **Payment Service**          | 处理支付（Stripe, PayPal, Amazon Pay）            |
> | **Notification Service**     | 发送确认邮件、推送通知                            |
> | **User Service**             | 用户身份管理（Cognito, OAuth）                    |
> | **Hotel Management Service** | 供酒店方管理房源、定价、可用性                    |
> | **Cache**                    | 加速查询（Redis, Memcached）                      |
> | **Logging & Monitoring**     | CloudWatch, ELK Stack 监控日志和错误              |
>
> ---
>
> ## **5. Database Selection（数据库选择）**
> | Data                     | Database                                         |
> | ------------------------ | ------------------------------------------------ |
> | **Hotel Metadata**       | **SQL (PostgreSQL, MySQL Aurora)** – 结构化数据  |
> | **Room Availability**    | **NoSQL (DynamoDB, Cassandra)** – 高吞吐，低延迟 |
> | **Bookings**             | **SQL (PostgreSQL, MySQL)** – 事务性预订         |
> | **Search Index**         | **Elasticsearch** – 处理全文搜索                 |
> | **User Data**            | **SQL (RDS, DynamoDB)**                          |
> | **Payment Transactions** | **SQL (ACID Required)**                          |
>
> ---
>
> ## **6. Table Design（表设计）**
...
> ---
>
> ## **7. System Workflow（基于服务/组件和技术的完整流程）**
> ### **1. 用户搜索酒店**
> - 用户请求 `GET /search?location=NYC&checkin=2025-05-01&checkout=2025-05-07`
> - API Gateway -> **Search Service** (Elasticsearch 查询) -> 返回结果
>
> ### **2. 预订房间**
> - 用户点击 "Book Now"
...
> - 使用 **distributed locking (Zookeeper, Redis Lock)** 处理并发预订  
> - 采用 **eventual consistency** 方式，允许小概率超售，通过 **waitlist 或退款机制** 解决
>
> ### **Q2: How do you scale the system?**
> - **Read-heavy services** (Search, Hotel Details) -> **Elasticsearch, Redis Cache**  
> - **Write-heavy services** (Bookings) -> **Sharding, CQRS (Command Query Responsibility Segregation)**  
> - **Event-Driven Architecture**: Kafka 处理异步任务（通知、支付）
>
> ---
>
> ## **9. How to Scale the System（如何扩展系统）**
> ### **1. Scale Read Operations**
> - **Cache layer (Redis/Memcached)** -> 缓存热门酒店数据  
> - **Read replicas** for SQL databases -> 复制数据库，提高读取性能  
> - **Elasticsearch for search** -> 高效支持全文搜索  
>
> ### **2. Scale Write Operations**
> - **Sharding database (分片存储)** -> 以 `hotel_id % N` 方式分片  
> - **Asynchronous processing (Kafka, SQS)** -> 处理支付、通知等任务  
>
> ### **3. Improve Availability**
> - **Multi-region deployments** -> AWS Global Load Balancer  
> - **CDN (CloudFront)** -> 静态内容加速  
>
> ---
>
> ### **总结**
> 这个设计平衡了 **高性能搜索、事务性预订、数据一致性**，同时使用 **分布式缓存、异步处理、数据库分片** 进行扩展。你可以根据具体的 follow-up 问题深入细节，比如 **CAP Theorem, Eventual Consistency, CQRS, Sharding Strategy** 等。
>

### 32. Design Weather Sensor System

> ## **System Design: Weather Sensor System**
> 这套系统的核心在于 **大规模数据写入（Write-heavy）**，并结合 **查询天气数据**（读取部分），我们重点关注 **传感器数据收集、存储、处理、查询优化、可扩展性** 以及 **高效数据写入策略**。
>
> ---
>
> ## **1. Functional Requirements（功能需求）**
> ### **Write-heavy (主要数据写入)**
> ✅ **Collect real-time weather data from sensors** – 来自世界各地的传感器，每秒上传温度、湿度、风速等数据。  
> ✅ **Store sensor data efficiently** – 需要存储大量传感器数据，并保证高效写入。  
>
> ### **Read (查询天气数据)**
> ✅ **View weather by clicking on a map** – 用户点击地图上的某个点，获取该位置当前温度、当日最高/最低温度。  
> ✅ **Historical weather trends** – 允许用户查询过去 24 小时、7 天、30 天的温度变化趋势（可选）。  
>
> ---
>
> ## **2. Non-Functional Requirements（非功能需求）**
> ✅ **High Write Throughput（高写入吞吐量）** – 支持数百万个传感器每秒上传数据。  
> ✅ **Low Read Latency（低读取延迟）** – 用户点击地图时，必须在 **<100ms** 内返回天气数据。  
> ✅ **Scalability（可扩展性）** – 能支持传感器数据增长，不受地域限制。  
> ✅ **Fault Tolerance（容错性）** – 传感器数据上传不能丢失，支持多数据中心部署。  
> ✅ **Data Consistency（数据一致性）** – 读写策略需要在 **高吞吐与数据一致性** 之间找到平衡。  
>
> ---
>
> ## **3. QPS & Storage Calculation（QPS 及存储计算）**
> ### **假设**
> - **Number of sensors（传感器数量）**: 10M
> - **Write frequency（每个传感器数据写入频率）**: 每 10 秒上报一次
> - **Data per write（每条写入数据大小）**: 200B
> - **Read QPS（用户查询 QPS）**: 10K QPS
>
> ### **QPS Calculation**
> - **Write QPS** = 10M sensors / 10s = **1M QPS**  
> - **Read QPS** = 10K（假设有 10K 并发查询天气数据的用户）
>
> ### **Storage Calculation**
> - **Daily writes** = 1M QPS * 86400s = **86.4B records/day**
> - **Daily storage** = 86.4B * 200B = **17.3 TB/day**
> - **Yearly storage** = 17.3 TB * 365 = **6.3 PB/year** (超大规模存储需求)
>
> ---
>
> ## **4. Services/Components Needed（所需服务/组件）**
> | Service                    | Function                                                 |
> | -------------------------- | -------------------------------------------------------- |
> | **API Gateway**            | 统一管理传感器写入、用户查询请求                         |
> | **Data Ingestion Service** | 处理传感器数据写入（Kafka, Kinesis）                     |
> | **Storage System**         | 存储传感器数据（Time-Series DB, S3 for cold storage）    |
> | **Query Service**          | 处理用户查询天气数据（Redis + Precomputed Aggregations） |
> | **Map Service**            | 地图交互界面，支持用户点击查询                           |
> | **Streaming Analytics**    | 实时计算最高/最低温度（Flink, Spark Streaming）          |
> | **Monitoring & Logging**   | 监控数据完整性、系统健康状态（CloudWatch, Prometheus）   |
>
> ---
>
> ## **5. Database Selection（数据库选择）**
> | Data                               | Database                                              |
> | ---------------------------------- | ----------------------------------------------------- |
> | **Real-time sensor data**          | **Time-Series DB (InfluxDB, TimescaleDB, Cassandra)** |
> | **Processed weather aggregations** | **Redis / DynamoDB** (低延迟读取)                     |
> | **Historical sensor data**         | **AWS S3 / Hadoop HDFS** (Cold Storage)               |
> | **User interactions**              | **PostgreSQL / MySQL (RDS, Aurora)**                  |
>
> ---
>
> ## **6. Table Design（表设计）**
...
> ```
>
> ### **Aggregated Weather Data (预计算数据)**
> ```sql
> CREATE TABLE WeatherAggregations (
...
>
> ## **7. System Workflow（基于服务/组件和技术的完整流程）**
> ### **1. 传感器数据收集（Write-heavy）**
> 1. **传感器** 每 10 秒上报数据至 **API Gateway**
> 2. **API Gateway** 通过 **Kafka/Kinesis** 将数据流式传输到 **Data Ingestion Service**
> 3. **Data Ingestion Service** 处理数据：
>    - **直接写入 Time-Series Database (TimescaleDB, Cassandra)**
>    - **存入 S3 进行长期存储**
>    - **触发 Streaming Analytics 计算 min/max/avg**
> 4. **Streaming Analytics (Flink/Spark Streaming)** 计算 24 小时最高/最低温，存入 **Redis**
>
> ---
>
> ### **2. 用户点击地图查询天气**
...
> 3. **Query Service**:
>    - 先从 **Redis** 查询缓存（当前温度、最高最低温）
>    - 若缓存未命中，则查询 **Time-Series Database**
> 4. **返回数据给用户**
>
> ---
>
> ## **8. Potential Interview Questions & Answers（可能的面试问题及解答）**
> ### **Q1: How do you handle high write throughput?**
> **Answer:**  
> - 采用 **Kafka / Kinesis** 作为 **buffering layer**，避免数据库压力过大  
> - 选择 **Time-Series DB (Cassandra, InfluxDB)**，支持高写入吞吐  
> - 使用 **batch writes (批量写入)** 提高吞吐量  
>
> ### **Q2: How do you handle real-time aggregation?**
> **Answer:**  
> - 使用 **Flink / Spark Streaming** 进行流式计算  
> - 计算 **当前温度、最高温、最低温** 并存入 **Redis**，确保查询低延迟  
>
> ### **Q3: How do you store historical data efficiently?**
> **Answer:**  
> - 采用 **hot-warm-cold storage** 架构：  
>   - **最近 7 天数据** 存在 **Time-Series DB**（查询快）  
>   - **7 天 – 1 年数据** 归档至 **S3 / HDFS**  
>   - **1 年以上数据** 进行压缩存储，减少存储成本  
>
> ---
>
> ## **9. How to Scale the System（如何扩展系统）**
> ### **1. Scale Writes（扩展写入）**
> - **Partitioning strategy**: 以 **sensor_id, timestamp** 进行 **sharding**  
> - **Write buffering**: 采用 **Kafka / Kinesis** 进行缓冲，提高写入吞吐  
> - **Batch writes**: 聚合多个传感器数据后再写入数据库  
>
> ### **2. Scale Reads（扩展读取）**
> - **Redis caching**: 预计算最高/最低温度，直接存入缓存  
> - **Sharded query architecture**: 分片存储数据，查询时并行处理  
>
> ### **3. High Availability（高可用性）**
> - **Multi-region deployment**: 使用 **AWS Route 53** 进行流量路由  
> - **Replication**: 数据库采用 **Multi-AZ Replica** 确保数据可用  
>
> ---
>
> ## **总结**
> 这套 **Weather Sensor System** 设计主要解决 **大规模写入、高效查询、扩展性和数据一致性**。核心架构包含：
> ✅ **Kafka/Kinesis 处理高吞吐数据流**  
> ✅ **Time-Series DB 进行高效写入**  
> ✅ **Redis 缓存加速查询**  
> ✅ **Flink/Spark Streaming 计算 min/max 预聚合数据**  
> ✅ **S3/Cold Storage 进行历史归档**  
>
> 如果你有更具体的 follow-up 问题，我们可以继续深入讨论，比如 **数据分片策略、事件驱动架构、读写优化**！🚀

### 32. Design HighWay Toll gate

> # **System Design: 高速公路收费站系统（Toll Collection System）**
>
> 我们按照你的结构来详细设计 **Toll Collection System**，重点关注 **高吞吐量、高可用性、低延迟和数据一致性**。
>
> ---
>
> ## **1. Functional Requirements（功能需求）**
...
>    - **ETC 电子标签识别**（RFID 设备读取 ETC 设备）  
>    - **ANPR（Automatic Number Plate Recognition，车牌自动识别）**  
>
> ✅ **Toll Calculation（通行费计算）**  
>    - 根据 **入口/出口站、行驶里程、车型、时段、优惠** 计算收费金额  
>
> ✅ **Payment Processing（支付处理）**  
>    - **ETC 自动扣费**（账户绑定信用卡/充值支付）  
>    - **电子支付（Alipay, WeChat Pay, Apple Pay）**  
>    - **人工现金/信用卡支付**（适用于人工收费通道）  
>
> ✅ **Violation Detection（逃费检测）**  
>    - 识别未支付车辆  
>    - 记录黑名单车辆，通知执法部门  
>
...
>    - **管理员账户**（监控收费站状态、管理设备）  
>
> ✅ **Real-time Traffic Monitoring（实时流量监控）**  
>    - 记录每个收费站的流量情况，避免拥堵  
>    - 预测流量，进行动态调度  
>
> ---
>
> ## **2. Non-Functional Requirements（非功能需求）**
> ✅ **High Throughput（高吞吐量）** – 需要支持 **百万级车辆并发通过**  
> ✅ **Low Latency（低延迟）** – ETC 交易必须在 **<100ms** 内完成，防止收费站拥堵  
> ✅ **Scalability（可扩展性）** – 需要支持全国范围的 **数万个收费站 & 数亿用户**  
> ✅ **High Availability（高可用性）** – 99.99% SLA，支持 **多数据中心部署**，防止系统宕机影响收费  
> ✅ **Data Consistency（数据一致性）** – 保证 **车辆进出站数据 & 账户扣费** 强一致性，防止重复扣费  
>
> ---
>
> ## **3. QPS & Storage Calculation（QPS 及存储计算，展示计算过程）**
>
> ### **假设**
> - **全国高速公路车辆总数（Total Vehicles）**: 300M  
> - **每日活跃车辆数（Daily Active Vehicles）**: 100M  
> - **每辆车进出站记录（Daily Toll Records per Vehicle）**: 2  
> - **全国收费站总数（Total Toll Stations）**: 10,000  
> - **摄像头 / ETC 识别频率（Sensor Data Frequency）**: 1000 次/秒/收费站  
> - **每条收费交易记录大小（Transaction Record Size）**: 500B  
>
> ### **QPS 计算**
> - **Toll Transactions QPS（收费交易 QPS）**:  
>   \[
>   \frac{100M * 2}{24 * 60 * 60} ≈ 2,300 QPS
>   \]
> - **ETC & Camera QPS（ETC/摄像头识别 QPS）**:  
>   \[
>   10,000 * 1,000 = 10M QPS
>   \]
> - **Payment Processing QPS（支付处理 QPS）**:  
>   \[
>   \frac{100M}{12 * 60 * 60} ≈ 2,300 QPS
>   \]
>
> ### **存储计算**
> - **Daily Toll Transactions（每日收费交易数据量）**:  
>   \[
>   100M * 2 = 200M
>   \]
> - **Daily Storage（每日存储需求）**:  
>   \[
>   200M * 500B = 100GB/day
>   \]
> - **Annual Storage（年度存储需求）**:  
>   \[
>   100GB * 365 = 36.5TB/year
>   \]
> - **Camera Image Storage（摄像头图片存储）**:  
>   \[
>   100M * 2MB = 200TB/day
>   \]
>   \[
...
> | Service                        | Function                                  |
> | ------------------------------ | ----------------------------------------- |
> | **API Gateway**                | 统一管理 API 请求（进出站、支付、查询）   |
> | **ETC & ANPR Service**         | 识别车辆（ETC 电子标签 / 车牌识别摄像头） |
> | **Toll Calculation Service**   | 计算通行费用                              |
> | **Payment Service**            | 处理支付（ETC/电子支付/人工支付）         |
> | **Fraud Detection Service**    | 识别逃费车辆                              |
> | **User Account Service**       | 用户账户管理（充值、支付、查询账单）      |
> | **Traffic Monitoring Service** | 监控收费站的流量情况，避免拥堵            |
> | **Data Storage Service**       | 记录车辆进出站数据，长期存储              |
> | **Logging & Monitoring**       | 监控系统健康状态，记录异常情况            |
>
> ---
>
> ## **5. Database Selection（数据库选择，列出所有需要的表）**
> | Data                            | Database                                     |
> | ------------------------------- | -------------------------------------------- |
> | **Vehicles**                    | **SQL (PostgreSQL, MySQL Aurora)**           |
> | **Users & Accounts**            | **SQL (PostgreSQL, MySQL Aurora)**           |
> | **Toll Transactions**           | **SQL (PostgreSQL, MySQL Aurora)**           |
> | **Real-time vehicle logs**      | **NoSQL (Cassandra, DynamoDB, TimescaleDB)** |
> | **Processed toll transactions** | **Redis / DynamoDB (低延迟查询)**            |
> | **Payment Records**             | **SQL (ACID Transactions Required)**         |
> | **Historical logs & Images**    | **S3 / HDFS（长期存储）**                    |
>
> ---
>
> ## **6. Table Design（表设计）**
...
> ### **1. 车辆进站**
> 1. **ETC 读取车辆信息**  
> 2. **API Gateway -> ETC Service** 验证车辆信息  
> 3. **记录 Entry Data** 到数据库  
>
> ---
>
> ### **2. 车辆出站 & 计算费用**
> 1. **ETC / Camera 识别车辆**  
> 2. **Toll Calculation Service** 计算收费  
> 3. **自动扣费**  
> 4. **更新数据库 Transaction Status**  
>
> ---
>
> ## **8. Interview Questions & Answers（可能的面试问题及解答）**
> ### **Q1: How do you handle high write throughput?**
> - **Kafka / Kinesis** 进行数据缓冲  
> - **DynamoDB / Cassandra** 高吞吐数据库  
>
...
>
> ## **9. How to Scale the System（如何扩展系统）**
> ✅ **Redis caching**: 存储热门查询结果  
> ✅ **Read replicas**: 复制数据库，提高查询速度  
> ✅ **Sharding Strategy**: 以 `station_id` 进行分片  
> ✅ **Multi-region deployments**: 提高可用性  
>
> ---
>
> 💡 这套系统设计确保 **高吞吐、低延迟、可扩展性**，你是否有任何改进建议？🚀

### 33 **银行间交易限额检查系统（Interbank Limit Checker）**
